...
- You'll need your own API keys for OpenAI/Gemini/DashScope
...
First, rename `env` to `.env` and add your API keys:
...
- DASHSCOPE_API_KEY (for Alibaba Cloud Qwen service)
- JINA_API_KEY (for Jina AI services)

These keys are required because the application supports multiple LLM providers for flexibility.
...
- `dashscope_qwen_turbo` - Uses Alibaba Cloud's Qwen Turbo model (requires DASHSCOPE_API_KEY)
