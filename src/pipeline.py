...
from src.weaviate_retrieval import WeaviateRetriever, WeaviateHybridRetriever
...
class PipelineConfig:
    def __init__(self, root_path: Path, subset_name: str = "subset.csv", questions_file_name: str = "questions.json", pdf_reports_dir_name: str = "pdf_reports", serialized: bool = False, config_suffix: str = ""):
        ...
        self.weaviate_db_path = self.databases_path / "weaviate_dbs"  # 新增：Weaviate数据库路径（用于配置文件等）
...
class RunConfig:
    ...
    use_weaviate_db: bool = False  # 新增：使用Weaviate数据库
    weaviate_url: str = "http://localhost:8080"  # Weaviate服务地址
    weaviate_api_key: str = None  # Weaviate API密钥
    weaviate_class_name: str = "DocumentChunk"  # Weaviate类名
...
class Pipeline:
    ...
    def export_reports_to_markdown(self, file_name):
        """
        使用 pdf_mineru.py，将指定 PDF 文件转换为 markdown 和结构化 JSON，并放到相应目录下。
        ...
        # 检查是否有JSON格式的结构化数据（包含页码等信息）
        json_path = os.path.join(extract_dir, "full.json")
        ...
        # 优先保存JSON格式的完整结构化数据到parsed_reports路径
        if os.path.exists(json_path):
            if not hasattr(self, 'parsed_reports_path'):
                self.paths.parsed_reports_path = self.paths.debug_data_path / "01_parsed_reports"
            
            os.makedirs(self.paths.parsed_reports_path, exist_ok=True)
            target_json_path = os.path.join(self.paths.parsed_reports_path, f"{base_name}.json")
            shutil.move(json_path, target_json_path)
            print(f"已将结构化数据 {json_path} 移动到 {target_json_path}")
        else:
            print(f"未找到JSON格式的结构化数据: {json_path}")
        
        # 同时保存markdown格式到markdown路径
        if os.path.exists(md_path):
            target_md_path = os.path.join(self.paths.reports_markdown_path, f"{base_name}.md")
            shutil.move(md_path, target_md_path)
            print(f"已将 {md_path} 移动到 {target_md_path}")
        else:
    ...
    def chunk_reports(self, include_serialized_tables: bool = False):
        """
        将报告分块，优先处理JSON格式保留页码信息，其次处理Markdown格式
        """
        text_splitter = TextSplitter()
        
        # 检查是否有JSON格式的结构化数据（来自MinerU解析结果）
        parsed_reports_path = self.paths.debug_data_path / "01_parsed_reports"
        if parsed_reports_path.exists() and any(parsed_reports_path.glob("*.json")):
            print(f"发现JSON格式的结构化数据，使用完整文档分块方法...")
            # 调用新的JSON处理方法
            text_splitter.split_json_reports(
                all_json_dir=parsed_reports_path,
                output_dir=self.paths.documents_dir,
                subset_csv=self.paths.subset_path
            )
        elif self.paths.reports_markdown_path.exists() and any(self.paths.reports_markdown_path.glob("*.md")):
            # 退回到原有的markdown处理
            print(f"使用Markdown格式分块 {self.paths.reports_markdown_path} 目录下的文件...")
            # 自动推断公司名称
            ...
            print("未找到可处理的JSON或Markdown文档")
            return
        
        print(f"分块完成，结果已保存到 {self.paths.documents_dir}")
    ...
    def create_weaviate_db(self):
        """从分块报告创建Weaviate数据库"""
        try:
            weaviate_ingestor = WeaviateDBIngestor(
                url=self.run_config.weaviate_url,
                api_key=self.run_config.weaviate_api_key
            )
            weaviate_ingestor.process_reports(
                all_reports_dir=self.paths.documents_dir,
                class_name=self.run_config.weaviate_class_name
            )
            print(f"Weaviate database created using URL: {self.run_config.weaviate_url}")
            return weaviate_ingestor
        except Exception as e:
            print(f"创建Weaviate数据库失败: {e}")
            print("请确保已安装weaviate-client: pip install weaviate-client")
            print("以及已启动Weaviate服务")
            return None
    ...
    def process_questions(self):
        ...
        processor = QuestionsProcessor(
            ...
            use_weaviate=self.run_config.use_weaviate_db,
            weaviate_url=self.run_config.weaviate_url,
            weaviate_api_key=self.run_config.weaviate_api_key,
            weaviate_class_name=self.run_config.weaviate_class_name
        )
